...
export default function ContactForm() {
  ...
  return (
    <div>
      {status === "success" && (
        <div className="rounded-none bg-green-50 border border-green-200 p-10 text-center mb-8">
      ...
      {status === "error" && (
        <div className="rounded-none bg-red-50 border border-red-200 p-6 text-center mb-8">
      ...
      <form ref={formRef} onSubmit={handleSubmit} className="space-y-8">
        ...
        <button
          ...
          className="bg-navy px-10 py-4 text-sm font-semibold tracking-wide uppercase text-white hover:bg-navy-light transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
        >
          {status === "submitting" ? "Sending..." : t("contactForm.submit")}
        </button>
      </form>
    </div>
  );
