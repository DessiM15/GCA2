      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div
          className={`flex items-center transition-all duration-500 ${
            scrolled ? "h-40 justify-center" : "h-44 justify-between"
          }`}
        >
          {scrolled ? (
            <>
              <div className="hidden md:flex items-center gap-8 flex-1 justify-end pr-10">
                {leftLinks.map((link) => (
                  <Link
                    key={link.href}
                    href={link.href}
                    className="text-[15px] font-medium tracking-wide uppercase text-white/80 hover:text-white transition-colors"
                  >
                    {link.label}
                  </Link>
                ))}
            ...
            <>
              <Link href="/" className="flex items-center gap-3">
                <Image
                  ...
                  height={170}
                />
                <span className="hidden sm:block text-2xl font-bold text-white drop-shadow-lg">
                  Gulf Coast Alloys
                </span>
              </Link>
              ...
              <div className="hidden md:flex items-center gap-8">
                {allLinks.map((link) => (
                  <Link
                    key={link.href}
                    href={link.href}
                    className="text-[15px] font-medium tracking-wide uppercase text-white/90 hover:text-white transition-colors drop-shadow-sm"
                  >
                    {link.label}
                  </Link>
                ))}
